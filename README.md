...
这是一个面向 Windows 10 的 `AutoHotkey v2` 脚本。它会执行两类定时操作：

- 在每小时 `40` 分附近检查传奇游戏窗口，在左上区域识别“立即传送”按钮并自动点击
- 每小时按一次你配置好的背包位置，双击一个双倍经验卡
...
4. 检查 `config.ini`：
   ...
   - 如果要启用经验卡功能，把 `[ExpCard]` 的 `Enabled=1`，并填写 `RelativeX/RelativeY`
...
`[ExpCard]` 段使用“相对游戏窗口左上角”的坐标：

- `Enabled`：`1` 为启用，`0` 为关闭
- `ActiveMinute`：每小时第几分钟触发，默认 `0`
- `PreTriggerSeconds/PostTriggerSeconds`：触发时间窗口
- `RelativeX/RelativeY`：经验卡在游戏窗口中的相对坐标
- `DoubleClickIntervalMs`：两次点击间隔，默认 `120` 毫秒

建议先把游戏窗口放到固定位置，鼠标移到经验卡中心，记录它相对于游戏窗口左上角的坐标，再回填到 `RelativeX/RelativeY`。
...
- 经验卡功能当前按固定坐标双击，不做图像识别，所以背包位置需要保持稳定
